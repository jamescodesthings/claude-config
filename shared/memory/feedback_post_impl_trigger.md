---
name: feedback-post-impl-trigger
...
**How to apply:** Treat the trigger as file-write-based, not agent-type or file-type based. Also: orchestrator must always delegate file writes to subagents (never write inline), so the trigger fires consistently. See [[feedback-subagent-preference]].
