...
DEBUG=0
DEBUG_LOG_DIR="$HOME/.gemini/status-logs"
...
USE_ASCII="${ANTIGRAVITY_STATUSLINE_ASCII:-0}"
USE_NERDFONT="${ANTIGRAVITY_STATUSLINE_NERDFONT:-0}"
USE_TRUECOLOR=0
...
model_color() {
  ...
  case "$1" in
    *Gemini*|*gemini*)  truecolor='\033[38;2;66;133;244m';  fallback="$CYAN" ;;
    ...
    *GPT*|*gpt*|*o1*|*o3*) truecolor='\033[38;2;16;163;127m'; fallback="$GREEN" ;;
...
# ═══════════════════════════════════════════════════════════════
# Read JSON (single jq invocation for Antigravity)
# ═══════════════════════════════════════════════════════════════
...
parsed=$(echo "$input" | jq -r '
  ...
  (.worktree.branch // .vcs.branch // ""),
  (.model.id // .model.display_name // "" | ascii_downcase) as $m |
  (if ($m | contains("gemini")) then "gemini" else "3p" end) as $qprefix |
  (($qprefix + "-5h") as $k |
    if .quota[$k] != null then
      ((1 - (.quota[$k].remaining_fraction // 1)) * 100 | tostring)
    elif .rate_limits.five_hour != null then
  ...
  (($qprefix + "-weekly") as $k |
    if .quota[$k] != null then
      ((1 - (.quota[$k].remaining_fraction // 1)) * 100 | tostring)
    elif .rate_limits.seven_day != null then
  ...
  (.model.effort // .effort.level // ""),
  (.workspace.project_dir // .workspace.repo.name // "." | split("/") | last),
  (($qprefix + "-5h") as $k |
    if .quota[$k] != null then
      ((now + (.quota[$k].reset_in_seconds // 0)) | floor | tostring)
    elif .rate_limits.five_hour != null then
...
GIT_CACHE="/tmp/antigravity-statusline-git-cache-${git_cache_key}"
GIT_CACHE_MAX_AGE=5
...
# ═══════════════════════════════════════════════════════════════
# Build second line: C, 5h, 7d bars
# ═══════════════════════════════════════════════════════════════
